...
class Game:
    def __init__(self, args: Dict[str, Union[str, int, bool]]):
        ...
        # Set up our robot
        self.robot = Strategy.make(env=self.env, strategy=self.args.get("model"), train=self.args.get("train"))
        self.robot.name = "robot"

        # Set up our opponent. The opponent can also be a human player.
        if self.args.get("human"):
            self.opponent = Agent(self.env, train=self.args.get("play"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--play", action="store_true", help="Play game instead of train")
