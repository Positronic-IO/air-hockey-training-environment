...
class LineShape(BaseShape):
    ...
    def calc_endpoints(self):
        ...
        if all(hasattr(self, attr) for attr in
               ['x', 'length', 'ang']):  # Can only calculate these if x, length, and ang are set
            # Need to set using self.__dict__ to avoid recursively calling self.__setattr__
            self.__dict__['x0'] = np.array([self.x[0] - self.length / 2 * np.cos(self.ang), self.x[1] - self.length / 2 * np.sin(self.ang)])
            self.__dict__['x1'] = np.array([self.x[0] + self.length / 2 * np.cos(self.ang), self.x[1] + self.length / 2 * np.sin(self.ang)])
    ...
    def calc_line_parameters(self):
        ...
        """
        if all(hasattr(self, attr) for attr in
               ['x0', 'x1']):  # Can only calculate these if x0 and x1 are both set
            x0 = self.x0[0]
            y0 = self.x0[1]
            x1 = self.x1[0]
            y1 = self.x1[1]
